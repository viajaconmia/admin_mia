...
const buildReservasRows = (reservas: ReservaAPI[] = []) => {
  return reservas.map((r, idx) => ({
    ...
    monto_items_pagos: r.monto_items_pagos ?? 0,
    codigo_confirmacion:
      r.codigo_confirmacion || r.codigo_reservacion_hotel || "N/A",
    item: r,
  }));
};
...
const ModalDetallePago: React.FC<ModalDetallePagoProps> = ({
  ...
  const [buscarReserva, setBuscarReserva] = React.useState("");
const [buscarFactura, setBuscarFactura] = React.useState("");
const [soloFacturas, setSoloFacturas] = React.useState(false);
const [ocultarFacturas, setOcultarFacturas] = React.useState(false);
...
const reservaRenderers = React.useMemo(() => {
  return {
    ...
    monto_items_pagos: CurrencyCell,
...
const reservasRowsFiltradas = React.useMemo(() => {
  const q = normalizeText(buscarReserva);
  if (!q) return reservasRows;

  return reservasRows.filter((r: any) => {
    const uuidReserva = normalizeText(
      r?.uuid_reserva ??
        r?.uuid ??
        r?.item?.uuid_reserva ??
        r?.item?.uuid ??
    ...
    const codigoConfirmacion = normalizeText(
      r?.codigo_confirmacion ??
        r?.confirmation_code ??
        r?.item?.codigo_confirmacion ??
        r?.item?.codigo_reservacion_hotel
    );

    return uuidReserva.includes(q) || codigoConfirmacion.includes(q);
  });
}, [reservasRows, buscarReserva, normalizeText]);

const facturasFiltradas = React.useMemo(() => {
  const q = normalizeText(buscarFactura);
  if (!q) return facturas;

  return facturas.filter((f: any) => {
    const uuid = normalizeText(f?.uuid_factura ?? f?.uuid_cfdi);
    const idFactura = normalizeText(f?.id_factura);
    return uuid.includes(q) || idFactura.includes(q);
  });
}, [facturas, buscarFactura, normalizeText]);

const facturaIdsFiltrados = React.useMemo(() => {
  const q = normalizeText(buscarFactura);
  if (!q) return facturaIds;

  return facturaIds.filter((id) => normalizeText(id).includes(q));
}, [facturaIds, buscarFactura, normalizeText]);

const totalReservasAsociadas = reservasRows.length;
const totalFacturasAsociadas =
  facturas.length > 0 ? facturas.length : facturaIds.length;

const totalReservasVisibles = reservasRowsFiltradas.length;
const totalFacturasVisibles =
  facturas.length > 0 ? facturasFiltradas.length : facturaIdsFiltrados.length;
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <h3 className="text-base font-semibold text-gray-800">
                Filtros de asociaciones
              </h3>
              <p className="text-sm text-gray-600">
                Reservas visibles: {soloFacturas ? 0 : totalReservasVisibles} / {totalReservasAsociadas}
              </p>
              <p className="text-sm text-gray-600">
                Facturas visibles: {ocultarFacturas ? 0 : totalFacturasVisibles} / {totalFacturasAsociadas}
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3">
              <label className="inline-flex items-center gap-3 select-none cursor-pointer">
                <span className="text-sm font-medium text-gray-700">
                  Ocultar reservas asociadas
                </span>

                <button
                  type="button"
                  role="switch"
                  aria-checked={soloFacturas}
                  onClick={() => setSoloFacturas((prev) => !prev)}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                    soloFacturas ? "bg-blue-600" : "bg-gray-300"
                  }`}
                >
                  <span
                    className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${
                      soloFacturas ? "translate-x-5" : "translate-x-1"
                    }`}
                  />
                </button>
              </label>
              ...
              <label className="inline-flex items-center gap-3 select-none cursor-pointer">
                <span className="text-sm font-medium text-gray-700">
                  Ocultar facturas asociadas
                </span>

                <button
                  type="button"
                  role="switch"
                  aria-checked={ocultarFacturas}
                  onClick={() => setOcultarFacturas((prev) => !prev)}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                    ocultarFacturas ? "bg-red-600" : "bg-gray-300"
                  }`}
                >
                  <span
                    className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${
                      ocultarFacturas ? "translate-x-5" : "translate-x-1"
                    }`}
                  />
                </button>
              </label>
            ...
            {!soloFacturas && (
              <div>
                ...
                <input
                  type="text"
                  value={buscarReserva}
                  onChange={(e) => setBuscarReserva(e.target.value)}
                  placeholder="Ej. UUID o código de confirmación"
            ...
            {!ocultarFacturas && (
              <div>
                ...
                <input
                  ...
                  placeholder="Ej. UUID de factura"
                  className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                />
              </div>
            )}
          ...
          {soloFacturas && ocultarFacturas && (
            <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3">
              <p className="text-sm font-medium text-amber-800">
                Las reservas y las facturas están ocultas.
              </p>
              <p className="text-xs text-amber-700 mt-1">
                Desactiva alguno de los switches para volver a mostrarlas.
              </p>
            </div>
          )}
        </div>
          ...
          <div className="mb-6">
            <div className="mb-3">
              ...
              <p className="text-sm text-gray-500">
                Mostrando {totalReservasVisibles} de {totalReservasAsociadas} reservas asociadas
              </p>
            </div>

            {!hasFetchedDetalles || loadingDetalles ? (
              <p className="text-sm text-gray-500">Cargando...</p>
            ) : reservasRowsFiltradas.length > 0 ? (
              <Table4
                registros={reservasRowsFiltradas}
                renderers={reservaRenderers as any}
                leyenda={`Mostrando ${totalReservasVisibles} de ${totalReservasAsociadas} reservas asociadas`}
                customColumns={[
                  "proveedor",
                  "tipo_cuarto_vuelo",
                  "viajero",
              ...
              <div className="border border-gray-200 bg-gray-50 rounded-lg p-4">
                ...
                <p className="text-xs text-gray-500 mt-1">
                  No se encontraron reservas con ese UUID o código de confirmación.
                </p>
              </div>
            ) : (
              <div className="border border-gray-200 bg-gray-50 rounded-lg p-4">
        ...
        {!ocultarFacturas && (
          <div className="mt-8">
            <div className="mb-3">
              ...
              <p className="text-sm text-gray-500">
                Mostrando {totalFacturasVisibles} de {totalFacturasAsociadas} facturas asociadas
              </p>
            </div>
            ...
            {!hasFetchedDetalles || loadingDetalles ? (
              ...
              facturasFiltradas.length > 0 ? (
                <div className="space-y-4">
                  {facturasFiltradas.map((f: any, idx: number) => {
                    ...
                    const montoAsociadoFactura = formatCurrency(
                      f.monto_asociado_factura
                    );
                            ...
                            <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
                              <div>
                                <div className="text-xs text-gray-500 font-medium">
                                  Monto asociado factura
                                </div>
                                <div className="text-sm font-semibold">
                                  {montoAsociadoFactura}
                                </div>
                              </div>
                              <div>
                                <div className="text-xs text-gray-500 font-medium">
                                  ...
                                  <div className="text-xs text-gray-700">
                                    {f.pagos_asociados.map((p: any, i: number) => (
                                      ...
                                      >
                                        ...
                                        <div>
                                          <b>Facturado:</b>{" "}
                                          {formatCurrency(p.monto_facturado ?? 0)}
                                        </div>
                                        <div>
                                          <b>Por facturar:</b>{" "}
                                          {formatCurrency(p.monto_por_facturar ?? 0)}
                                        </div>
                                      </div>
                                    ))}
                                  </div>
                ...
                <div className="border border-gray-200 bg-gray-50 rounded-lg p-4">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    No se encontraron facturas con ese UUID.
                  </p>
                </div>
              )
            ) : facturaIds.length > 0 ? (
              facturaIdsFiltrados.length > 0 ? (
                <div className="space-y-2">
                  ...
                  {facturaIdsFiltrados.map((facturaId, idx) => {
                ...
                <div className="border border-gray-200 bg-gray-50 rounded-lg p-4">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    No se encontraron IDs de factura con ese filtro.
                  </p>
                </div>
              )
            ) : (

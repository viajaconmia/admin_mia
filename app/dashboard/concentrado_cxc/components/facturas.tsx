...
export const DetallesFacturas: React.FC<DetallesFacturasProps> = ({
        ...
        data.forEach((item: any) => {
          if (item.facturas_json && Array.isArray(item.facturas_json)) {
            facturasExtraidas.push(
              ...item.facturas_json.map((factura: any) => ({
                ...factura,
                diasRestantes: factura.diasRestantes || 0,
                diasCredito: factura.diasCredito || 0,
                nombre_agente:
                  item.nombre_agente || item.nombre || "Sin nombre",
              })),
            );
          }
        });
  ...
  const handleLinkPayment = async () => {
    try {
      const facturasSeleccionadas = facturas
        .filter((f) => selectedFacturas.has(f.id_factura))
        .map((f) => ({
          id_factura: f.id_factura,
          uuid_factura: f.uuid_factura,
          total: Number(f.total || 0),
          folio: f.folio || f.uuid_factura || f.id_factura,
          saldo: Number(f.saldo || 0),
          monto_a_pagar: Number(montosAsignar[f.id_factura] || 0),
        }));

      const total = facturasSeleccionadas.reduce(
        (acc, f) => acc + f.monto_a_pagar,
        0,
      );
      if (total <= 10)
        throw new Error("El monto a pagar debe ser mayor a $10.00.");

      setIsCreatingLink(true);
      setLinkPago(null);
      const { url } = await crearLinkPago(facturasSeleccionadas);
      setLinkPago(url);
    } catch (error: any) {
      showNotification(
        "error",
        error.message || "Error al crear el link de pago",
      );
    } finally {
  ...
  const registros = pagoData
    ...
    : (facturasFiltradas || []).map((f) => ({
        ...
        dias_a_credito: f.diasCredito || 0,
        dias_restantes:
          f.diasRestantes ?? calcDiasRestantes(f.fecha_vencimiento),
        monto_asignar: f,
        seleccionar: f,
        item: f,
      }));
            ...
            {isLoading ? (
              ...
              <div className="border rounded-lg overflow-hidden">
                ...
                <div className="p-2 space-y-3">
                  ...
                  {linkPago && (
                    <div className="rounded-lg border border-blue-300 bg-blue-50 px-4 py-4 text-center">
                      ...
                      <a
                        ...
                        className="text-blue-700 font-bold text-base break-all hover:underline"
                      >
                        {linkPago}
                      </a>
                      <button
                        ...
                        className="mt-3 block mx-auto text-xs text-blue-600 hover:underline"
                      >
                        Copiar link
                      </button>
                    </div>
                  ...
                  >
                    ...
                    <button
                      className="bg-gray-700 text-white px-3 py-1 rounded hover:bg-gray-800 disabled:bg-gray-400 disabled:cursor-not-allowed text-sm"
                      onClick={handleLinkPayment}
                      disabled={selectedFacturas.size === 0 || isCreatingLink}
                    >
                      {isCreatingLink ? "Generando..." : "Crear link de pago"}
                    </button>
                  </Table5>
                </div>
              </div>

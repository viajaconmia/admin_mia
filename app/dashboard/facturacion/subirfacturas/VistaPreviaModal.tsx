...
export default function VistaPreviaModal({
  ...
  const facturadoMap = useMemo(() => {
    ...
    if (
      facturadoData?.data_by_id &&
      typeof facturadoData.data_by_id === "object"
    ) {
    ...
    <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center">
      <div className="bg-white p-6 rounded shadow-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        {typeof itemsTotal === "number" && itemsTotal > 0 && (
          <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <p className="font-semibold">
                  {itemsTotal.toLocaleString("es-MX", {
                    style: "currency",
                    currency: "MXN",
                  })}
                </p>
            ...
            >
              {okItems ? (
                <p className="font-semibold">
                  Diferencia (factura - ítems):{" "}
                  {formatCurrency(diferenciaItems.toFixed(2), "MXN")}
                </p>
              ) : (
                <p className="font-semibold">
                  El total de los ítems excede el total de la factura. Ajusta la
                  selección o usa otra factura.
                </p>
              )}
            </div>
          </div>
        ...
        {isProveedorBatch &&
          ...
          Object.keys(facturadoMap).length === 0 && (
            <div className="mb-4 p-3 rounded border bg-yellow-50 text-yellow-700 text-sm">
              No se encontraron montos disponibles para las solicitudes
              seleccionadas.
            </div>
          )}
        ...
        {requiereConversionProveedor && (
          <div className="mb-4 p-4 bg-indigo-50 border border-indigo-200 rounded-lg text-sm">
            ...
            <div>
              <strong>Moneda original:</strong> {monedaFactura}
            </div>
            ...
            <div>
              ...
              {tipoCambioFactura || "No disponible"}
            </div>
            {modoTipoCambioManual && (
              <div className="mb-3">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setTipoCambioManualInput(safeTcStr(e.target.value))
                  }
            ...
            <div>
              <strong>Subtotal en MXN:</strong>{" "}
              {formatCurrency(subtotalFacturaMXN, "MXN")}
            </div>
            <div>
              <strong>Impuestos en MXN:</strong>{" "}
              {formatCurrency(impuestosFacturaMXN, "MXN")}
            </div>
          </div>
        )}
...
const FacturaEstructurada = ({
  ...
  <div className="border border-gray-200 rounded-lg p-6">
    ...
    <div className="mt-6 pt-4 border-t text-xs text-gray-500">
      ...
      <p>
        UUID:{" "}
        <span className="text-blue-500">{facturaData.timbreFiscal.uuid}</span>
      </p>
      <p>
        Fecha de timbrado: {formatDate(facturaData.timbreFiscal.fechaTimbrado)}
      </p>
    </div>
  </div>
);

...
async function exportCotizacionToExcel(
  rows: CotizacionRow[],
  providers: string[],
) {
  ...
  const BLUE = "FF42A5D4";
  const PROVIDER_BG = ["FF1E3A5F", "FF2C4A7A", "FF3D3072", "FF5A2070"];
  const WHITE_FONT = { argb: "FFFFFFFF" };
  const DARK_FONT = { argb: "FF1F2937" };
  ...
  const mkBorder = (): ExcelJS.Borders => ({
  ...
  const styleHeader = (cell: ExcelJS.Cell, bgArgb: string) => {
    ...
    cell.font = { bold: true, color: WHITE_FONT, size: 9 };
    cell.alignment = {
      horizontal: "center",
      vertical: "middle",
      wrapText: true,
  ...
  const styleSubHeader = (cell: ExcelJS.Cell, bgArgb: string) => {
    cell.fill = mkFill(bgArgb);
    cell.font = { bold: true, color: WHITE_FONT, size: 8 };
    cell.alignment = {
      horizontal: "center",
      vertical: "middle",
      wrapText: true,
  ...
  const styleData = (cell: ExcelJS.Cell, even: boolean) => {
    cell.fill = mkFill(even ? "FFFFFFFF" : "FFF9FAFB");
    cell.font = { color: DARK_FONT, size: 9 };
  ...
  const r1Detail = ws.getCell(1, 1);
  r1Detail.value = "DETALLE SERVICIOS";
  styleHeader(r1Detail, BLUE);

  providers.forEach((name, pi) => {
    const start = DETAIL_COLS + 1 + pi * PROV_COLS;
    ws.mergeCells(1, start, 1, start + PROV_COLS - 1);
    const cell = ws.getCell(1, start);
    cell.value = `COSTO PROVEEDOR ${name}`;
    styleHeader(cell, PROVIDER_BG[pi % PROVIDER_BG.length]);
  });
  ...
  detailHeaders.forEach((h, ci) => {
    ...
    cell.value = h;
    styleSubHeader(cell, BLUE);
  });

  providers.forEach((_, pi) => {
    provHeaders.forEach((h, hi) => {
      const cell = ws.getCell(2, DETAIL_COLS + 1 + pi * PROV_COLS + hi);
      cell.value = h;
      styleSubHeader(cell, PROVIDER_BG[pi % PROVIDER_BG.length]);
    });
  });
  ...
  rows.forEach((row, i) => {
    ...
    const values: (string | number)[] = [
      ...
      row.hotel,
      row.habitacion,
      row.desayuno === 1 ? "SÍ" : "NO",
      cant,
      row.checkin,
      row.checkout,
      row.noches,
      parseInt(row.noktos_unitarios) || 0,
      precio,
      importeTotal,
    ];
    ...
    providers.forEach((_, pi) => {
      ...
      const markup =
        precio > 0 ? `${(((precio - costo) / precio) * 100).toFixed(2)}%` : "-";
      const convenio = row.convenios[pi] ?? 0;
      const baseCol = DETAIL_COLS + 1 + pi * PROV_COLS;
      const provFill = PROVIDER_BG[pi % PROVIDER_BG.length].replace("FF", "10");

      const provValues: (string | number)[] = [
        convenio === 1 ? "SÍ" : "NO",
        costo,
        costoTotal,
        markup,
      ...
      provValues.forEach((v, hi) => {
        const cell = ws.getCell(excelRow, baseCol + hi);
        cell.value = v;
        cell.fill = mkFill(even ? "FFF0F4FF" : "FFE8EEFF");
        cell.font = { color: DARK_FONT, size: 9 };
        cell.alignment = { horizontal: "center", vertical: "middle" };
        cell.border = mkBorder();
        if (hi === 1 || hi === 2 || hi === 4) {
          ...
          if (hi === 4) {
            cell.font = {
              ...cell.font,
              bold: true,
              color: { argb: ganancia >= 0 ? "FF065F46" : "FF991B1B" },
            };
          }
        }
      });
  ...
  // ── Column widths ─────────────────────────────────────────────────────
  const colWidths = [4, 22, 14, 10, 8, 12, 12, 8, 10, 14, 14];
  colWidths.forEach((w, i) => {
    ws.getColumn(i + 1).width = w;
  });
  providers.forEach((_, pi) => {
    [10, 14, 14, 10, 14].forEach((w, hi) => {
      ws.getColumn(DETAIL_COLS + 1 + pi * PROV_COLS + hi).width = w;
    });
  });
...
function CotizacionModal({
  ...
  const [rows, setRows] = React.useState<CotizacionRow[]>(() =>
    slots
      ...
      .map((s) => {
        ...
        const costoMia =
          hotelMia?.tipos_cuartos[0]?.costo ?? s.costo_sistema ?? "0";
  ...
  const handleExport = async () => {
    ...
    try {
      await exportCotizacionToExcel(rows, providers);
    } finally {
    ...
    <Modal onClose={onClose} title="Cotización" subtitle="Detalle de servicios">
      <div className="flex justify-end mb-2">
        <Button
          size="sm"
          variant="secondary"
          icon={exportingXlsx ? Loader2 : Download}
          ...
          <thead>
            ...
            <tr>
              {[
                ...
                "IMPORTE / NOCHE",
                "IMPORTE TOTAL",
              ].map((h) => colHeader(h, h, "#42a5d4", "#2e8bbb"))}
                  ...
                  <td className="px-1 py-1 border border-gray-200 text-center">
                    <EditableCell
                      ...
                      onChange={(v) =>
                        update(i, {
                          desayuno:
                            v.toUpperCase() === "SÍ" || v.toUpperCase() === "SI"
                              ? 1
                              : 0,
                        })
                      }
                      ...
                      <React.Fragment key={pi}>
                        ...
                        >
                          <button
                            type="button"
                            onClick={() =>
                              updateConvenio(i, pi, convenio === 1 ? 0 : 1)
                            }
                        ...
                        <td
                          className={`px-1 py-1 border border-gray-200 ${c.cell}`}
                        >
                          <EditableCell

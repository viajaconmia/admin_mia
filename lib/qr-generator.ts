...
export async function generateSecureQRPaymentPDF(data: QRPaymentData) {
  ...
  doc.setTextColor(0, 0, 0);
  doc.text('• Este código QR es único y tiene validez limitada (30 minutos)', 25, 178);
  doc.text('• No comparta este documento con terceros', 25, 185);
  doc.text('• Los datos de la tarjeta solo son visibles al escanear el código', 25, 192);
  doc.text('• El acceso expira automáticamente por seguridad', 25, 199);
  
  // Footer
  doc.setFontSize(9);
  doc.setTextColor(100, 100, 100);
  doc.text('Documento generado automáticamente - Sistema de Facturación', 20, 270);
  doc.text(`Fecha: ${new Date().toLocaleDateString('es-MX')} - Hora: ${new Date().toLocaleTimeString('es-MX')}`, 20, 278);
...
export function generateSecureToken(reservationId: number, amount: number, cardType: string): string {
...
export function validateSecureToken(token: string): { valid: boolean; data?: any } {
  try {
    // Reverse the token generation process
    const decoded = atob(token.replace(/[-_]/g, (match) => {
      return { '-': '+', '_': '/' }[match] || match;
    }));

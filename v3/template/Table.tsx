...
const TableBookings = ({ filters, extras }:{filters, extras, loading}) => {
  ...
  const handleFetchSolicitudes = async (page = tracking.page) => {
    ...
    showNotification("error", error.message || "ocurrio un error al generar el filtrado")
  }
  };
  ...
  return (
    <>
      ...
      {creacion && (
        ...
        >
          <CrearReserva
            agente={{ id_agente: extras.id_agente } as Agente}
          ></CrearReserva>
        </Modal>
      )}
    </>

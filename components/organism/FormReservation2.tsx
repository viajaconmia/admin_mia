...
import { LoginScreen } from "@/app/ventas/page";
import {
  CheckboxInput,
  ComboBox,
  DateInput,
...
export function ReservationForm2({
  ...
  const [precioManual, setPrecioManual] = useState<number>(form.venta.total);

  useEffect(() => {
    console.log("Edicion FORM", edicionForm, viajero);
    setId_agente(edicionForm.metadata.id_agente || null);
    setPrecioManual(form.venta.total)
  }, [form]);
            ...
            <div className="grid md:grid-cols-3 gap-4">
              <div className="flex flex-col">
                <NumberInput
                  ...
                  value={precio}
                  onChange={(value: string) => setPrecioManual(Number(value))}
                />
                <button
                  type="button"
                  className="text-blue-600 text-sm mt-2 hover:underline hover:text-blue-900 transition-colors"
                  onClick={() => setPrecio(precioManual)}
                >
                  Precio recomendado: {precioManual}
                </button>
              </div>

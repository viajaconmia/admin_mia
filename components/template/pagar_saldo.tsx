  ...
  const fetchSaldoFavorData = async () => {
    try {
      ...
      response.data?.forEach((saldo: any) => {
        ...
        const idItem = reservaData
          ? `saldo-${saldo.id_saldos}`
          : `saldo - ${saldo.id_saldos}`;
        const saldoValor = reservaData
            ...
            <input
              ...
              onChange={() =>
                handleItemSelection(
                  ...
                  facturaData[0] != 0
                    ? Number(value.monto_por_facturar)
                    : Number(value.saldo)
                )
              }
